...
<%
	...
	if( dataTypeId > 0 ){
		isAdd = false;
		dataType = (DataType)renderRequest.getAttribute(IcecapDataTypeWebKeys.DATATYPE);
	}
...
<portlet:actionURL name="<%= IcecapDataTypeMVCCommands.ACTION_DATATYPE_CREATE_AND_DEFINE_STRUCTURE %>" var="addToDefineStructureActionURL">
	<portlet:param name="<%= IcecapDataTypeWebKeys.CMD %>" value="<%= IcecapDataTypeConstants.CMD_REDIRECT_TO_DEFINE_STRUCTURE %>"/>
	<portlet:param name="<%= IcecapDataTypeWebKeys.BACK_URL %>" value="<%= currentURL %>"/>
</portlet:actionURL>
...
<portlet:actionURL name="<%= IcecapDataTypeMVCCommands.ACTION_DATATYPE_DELETE %>" var="deleteActionURL">
	<portlet:param name="<%= IcecapDataTypeWebKeys.DATATYPE_ID %>" value="<%= String.valueOf(dataTypeId) %>"/>
</portlet:actionURL>

<portlet:renderURL var="defineStructureRenderURL">
	<portlet:param name="<%= IcecapDataTypeWebKeys.MVC_RENDER_COMMAND_NAME %>" value="<%= IcecapDataTypeMVCCommands.RENDER_DEFINE_DATA_STRUCTURE %>"/>
	<portlet:param name="<%= IcecapDataTypeWebKeys.DATATYPE_ID %>" value="<%= String.valueOf(dataTypeId) %>"/>
</portlet:renderURL>


<portlet:resourceURL 
		id="<%= IcecapDataTypeMVCCommands.RESOURCE_DATATYPE_RENDER_TERM_ATTR %>" 
		var="renderTermAttributes">
</portlet:resourceURL>

<aui:container cssClass="SXDataMarket-web">
	<aui:row>
		<aui:col md="2">
	...
	<aui:form method="post" name="dataTypeForm" id="dataTypeForm">
		<c:if test="<%= isUpdate %>">
