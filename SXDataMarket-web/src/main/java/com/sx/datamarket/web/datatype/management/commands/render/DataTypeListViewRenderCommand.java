...
import com.sx.datamarket.web.datatype.management.display.context.DataTypeManagementToolbarDisplayContext;
...
//		System.out.println("TermListViewRenderCommand.render()");
		...
		try {
			searchContainer = _createSearchContainer( 
					renderRequest,
					renderResponse,
					IcecapDataTypeConstants.SEARCH_CONTAINER_ID, 
		...
		return IcecapDataTypeConstants.JSP_DATATYPE_LIST_VIEW;
	}
	...
	private boolean _trySearchThroughCategoryTree(
		...
		long assetCategoryId = ParamUtil.getLong(renderRequest, IcecapDataTypeWebKeys.CATEGORY_ID);
		String assetTagName = ParamUtil.getString(renderRequest, IcecapDataTypeWebKeys.TAG);
		
		if ((assetCategoryId != 0) || Validator.isNotNull(assetTagName)) {
			AssetEntryQuery assetEntryQuery = new AssetEntryQuery(DataType.class.getName(), searchContainer);

			assetEntryQuery.setExcludeZeroViewCount(false);
			assetEntryQuery.setOrderByCol1(ParamUtil.getString(renderRequest, IcecapDataTypeWebKeys.ORDER_BY_COL));
			assetEntryQuery.setOrderByType1(ParamUtil.getString(renderRequest, IcecapDataTypeWebKeys.ORDER_BY_TYPE));
...
//			assetEntries.forEach(assetEntry -> entriesResults.add(_dataTypeLocalService.getDataType(assetEntry.getClassPK())));
	...
	private boolean _trySearchThroughService (
		...
		String Keywords = ParamUtil.getString( renderRequest, IcecapDataTypeWebKeys.KEYWORDS, StringPool.BLANK);
		if( !Keywords.isEmpty() ) {
			return false;
		}
		...
		else if( navigation.equals(IcecapDataTypeConstants.NAVIGATION_GROUP)){
			searchContainer.setTotal(
	...
	private boolean _trySearchWithIndexer(
		...
		ThemeDisplay themeDisplay = (ThemeDisplay)renderRequest.getAttribute(WebKeys.THEME_DISPLAY);
		String navigation = ParamUtil.getString( renderRequest, IcecapDataTypeWebKeys.NAVIGATION, IcecapDataTypeConstants.NAVIGATION_ALL);
		String orderByCol = ParamUtil.getString( renderRequest, IcecapDataTypeWebKeys.ORDER_BY_COL, IcecapDataTypeAttributes.DATATYPE_NAME);
		String orderByType = ParamUtil.getString( renderRequest, IcecapDataTypeWebKeys.ORDER_BY_TYPE, IcecapDataTypeConstants.ASC);
		int status = ParamUtil.getInteger( renderRequest, IcecapDataTypeAttributes.STATUS, WorkflowConstants.STATUS_APPROVED);
		...
		if (orderByType.equals(IcecapDataTypeConstants.DESC)) {
			reverseSort = true;
		}
	...
	private void _setSearchOptions(
		...
		searchContainer.setOrderByCol(
				ParamUtil.getString(
						renderRequest, 
						IcecapDataTypeWebKeys.ORDER_BY_COL, 
						IcecapDataTypeAttributes.DATATYPE_NAME));
		searchContainer.setOrderByType(
				ParamUtil.getString(
						renderRequest, 
						IcecapDataTypeWebKeys.ORDER_BY_TYPE, 
						IcecapDataTypeConstants.ASC));
